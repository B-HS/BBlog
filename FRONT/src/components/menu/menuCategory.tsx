...
const MenuCategory = ({ className, title, count, type }: MenuCategoryProps) => {
    const isActivated = true
    return (
        <Link href={type === 'PAGE' ? `/doc/${title.toLowerCase()}` : `/${title.toLowerCase()}`}>
            <Flex
            ...
            >
                ...
                {!!count && <span className='opacity-50 text-xs'>({count})</span>}
            </Flex>
        </Link>
    )
}
