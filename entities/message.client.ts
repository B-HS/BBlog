...
export const useInfiniteGetMessagesByUserId = (userId: string, size: number = 10) => {
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam }) => {
            return clientFetch<GetMessagesByUserIdResponse>(`/api/blog/messages/user/${userId}?page=${pageParam}&size=${size}`)
        },
        initialPageParam: 1,
...
export const useCreateMessage = (userId: string) => {
    ...
    return useMutation({
        mutationFn: async (data: { body: string; imageIds?: string[]; replyToId?: string; retweetOfId?: string }) => {
            return clientFetch('/api/blog/messages', {
                method: 'POST',
...
export const useDeleteMessage = (userId: string) => {
    ...
    return useMutation({
        mutationFn: async (messageId: string) => {
            return clientFetch(`/api/blog/messages/${messageId}`, {
                method: 'DELETE',
            })
        },
        onSuccess: () => {
